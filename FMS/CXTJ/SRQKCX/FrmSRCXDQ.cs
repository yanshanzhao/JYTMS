...
{
    public partial class FrmSRCXDQ : Form
    {
        #region 成员变量
        private ClsG ObjG;
        ...
        public void Prepare(string akssj, string ajssj, int ajgid, string aywqy, string ajgmc)
        {
            ...
            SQLstr = "SELECT  id5 as jgid,mc5 as jgmc,sum(ffsr) as ffsr ,sum(tfsr) as tfsr,sum(zfsr) as zfsr,sum(hfsr) as hfsr,sum(hjsr) as hjsr,sum(ffbf) as ffbf,sum(tfbf) as tfbf ,sum(zfbf) as zfbf,sum(hfbf) as hfbf,sum(hjbh) as hjbh,sum(zyf) as zyf,4 as level FROM  Vfmssrqkcx where id4=" + ajgid + awhere + " group by id5,mc5";
