...
{
    public partial class FrmLSDWFKDSK2 : Form
    {
        #region 成员变量
        private ClsG ObjG;
        private int PriDays;
        ...
        #endregion
        public FrmLSDWFKDSK2()
        {
            InitializeComponent();
        }
        public void Prepare(string ajgmc, int ajgid, decimal azj, int aLevel, int aLx1, DateTime sj)
        {
            PriLx1 = aLx1;
            ...
            using (SqlConnection conn = new SqlConnection(ClsGlobals.CntStrTMS))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("P_dskwfkcx7"))
                {
                    cmd.Connection = conn;
                    ...
                    if (dt.Rows.Count != 0)
                    {
                        ...
                        this.lbldbc.Text = this.lbldbc.Text + ":" + dr["dbc"].ToString() + "元";
                        dt.Rows.Add(dr);
                    }
                    else
        ...
        public void Prepare(int ajgid, DateTime Endsj, int Lx)
        {
            ...
            using (SqlConnection conn = new SqlConnection(ClsGlobals.CntStrTMS))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("P_dskwfkcx6"))
                {
                    cmd.Connection = conn;
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddRange(als.ToArray(typeof(SqlParameter)));
