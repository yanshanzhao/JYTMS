...
{
    public partial class FrmWfkDskCx1 : UserControl
    {
        #region 成员变量
        private ClsG ObjG;
        ...
        private DateTime PriStopXx = DateTime.Now;
        private int fklx = 0;
        #endregion
        public FrmWfkDskCx1()
        {
        ...
        private void BtnSearch_Click(object sender, EventArgs e)
        {
            ...
            string year = PriStop.ToString("yyyy - MM - dd");
            PriStopXx = Convert.ToDateTime(year + " " + hours + ":" + minute + ":" + second);
            fklx = CmbFklx.SelectedIndex + 1;
            ...
            using (SqlConnection conn = new SqlConnection(ClsGlobals.CntStrTMS))
            {
                ...
                als.Add(new SqlParameter("@fklx", fklx));
                als.Add(new SqlParameter("@sjXx", PriStopXx));
                using (SqlCommand cmd = new SqlCommand())
                {
                    cmd.CommandText = PriLevel < 3 ? "P_dskwfkcx3" : "P_dskwfkcx4";
                    cmd.Connection = conn;
        ...
        {
            PriStop = DtpStart.Value.Date;
            int day = PriStop.Date.Subtract(DateTime.Now.Date).Days;
            fklx = CmbFklx.SelectedIndex + 1;
            string SQLstr = "";
            if (fklx == 1)
            {
                SQLstr = " SELECT dqmc, bh, sljgmc, slsj, qsd, mdd, qsjgmc, fhf, shlxr, ddsj,(case when (yqts+" + PriDays + ")<=1 then 0 else (yqts+" + PriDays + "-1) end) as yqts, lx, dsk,fkfs,sjzt,sjsj,mc,zhmc,zh  FROM dbo.Vfmswfkdskcx where yqts+ " + PriDays + " >=0 and jgid in (select id from jyjckj.dbo.tjigou WHERE parentLst LIKE '%," + PriJgid + ",%' and fkfs='POS') ";
            }
            else if (fklx == 2)
            {
                SQLstr = " SELECT dqmc, bh, sljgmc, slsj, qsd, mdd, qsjgmc, fhf, shlxr, ddsj, (case WHEN DATEDIFF(hh,dateadd(hh,15,CONVERT(varchar(40),inssj, 23)),inssj)>0  AND DATEDIFF(day,inssj,'" + PriStop + "')<=1 then 0 WHEN DATEDIFF(hh,dateadd(hh,15,CONVERT(varchar(40),inssj, 23)),inssj)>0  AND DATEDIFF(day,inssj,'" + PriStop + "')> 1 then DATEDIFF(day,inssj,'" + PriStop + "')-1 WHEN DATEDIFF(hh,dateadd(hh,15,CONVERT(varchar(40),inssj, 23)),inssj)<=0 AND DATEDIFF(day,inssj,'" + PriStop + "')<=1 THEN 1 WHEN DATEDIFF(hh,dateadd(hh,15,CONVERT(varchar(40),inssj, 23)),inssj)<=0 AND DATEDIFF(day,inssj,'" + PriStop + "')> 1 THEN DATEDIFF(day,inssj,'" + PriStop + "') end) as yqts, lx, dsk,fkfs,sjzt,sjsj,mc,zhmc,zh  FROM dbo.Vfmswfkdskcx where yqts+ " + PriDays + " >=0 and jgid in (select id from jyjckj.dbo.tjigou WHERE parentLst LIKE '%," + PriJgid + ",%' and fkfs='现金') ";
            }
            else
            {
                SQLstr = " SELECT dqmc, bh, sljgmc, slsj, qsd, mdd, qsjgmc, fhf, shlxr, ddsj,(case when (yqts+" + PriDays + ")<=1 then 0 else (yqts+" + PriDays + "-1) end) as yqts, lx, dsk,fkfs,sjzt,sjsj,mc,zhmc,zh  FROM dbo.Vfmswfkdskcx where yqts+ " + PriDays + " >=0 and jgid in (select id from jyjckj.dbo.tjigou WHERE parentLst LIKE '%," + PriJgid + ",%' and fkfs='POS') UNION ALL SELECT dqmc, bh, sljgmc, slsj, qsd, mdd, qsjgmc, fhf, shlxr, ddsj, (case WHEN DATEDIFF(hh,dateadd(hh,15,CONVERT(varchar(40),inssj, 23)),inssj)>0  AND DATEDIFF(day,inssj,'" + PriStop + "')<=1 then 0 WHEN DATEDIFF(hh,dateadd(hh,15,CONVERT(varchar(40),inssj, 23)),inssj)>0  AND DATEDIFF(day,inssj,'" + PriStop + "')> 1 then DATEDIFF(day,inssj,'" + PriStop + "')-1 WHEN DATEDIFF(hh,dateadd(hh,15,CONVERT(varchar(40),inssj, 23)),inssj)<=0 AND DATEDIFF(day,inssj,'" + PriStop + "')<=1 THEN 1 WHEN DATEDIFF(hh,dateadd(hh,15,CONVERT(varchar(40),inssj, 23)),inssj)<=0 AND DATEDIFF(day,inssj,'" + PriStop + "')> 1 THEN DATEDIFF(day,inssj,'" + PriStop + "') end) as yqts, lx, dsk,fkfs,sjzt,sjsj,mc,zhmc,zh  FROM dbo.Vfmswfkdskcx where yqts+ " + PriDays + " >=0 and jgid in (select id from jyjckj.dbo.tjigou WHERE parentLst LIKE '%," + PriJgid + ",%' and fkfs='现金') ";
            ...
            try
            {
                ...
                row0.CreateCell(13).SetCellValue("付款方式");
                row0.Cells[13].CellStyle = styleRow;
                row0.CreateCell(14).SetCellValue("上缴状态");
                row0.Cells[14].CellStyle = styleRow;
                row0.CreateCell(15).SetCellValue("上缴时间");
                row0.Cells[15].CellStyle = styleRow;
                row0.CreateCell(16).SetCellValue("机构开户行");
                row0.Cells[16].CellStyle = styleRow;
                row0.CreateCell(17).SetCellValue("户名");
                row0.Cells[17].CellStyle = styleRow;
                row0.CreateCell(18).SetCellValue("账号");
        ...
        private void Dgv_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (PriLevel < 3)
            {
                FrmLsdWfkDsk1 f = new FrmLsdWfkDsk1();
                f.ShowDialog();
                f.Prepare(ajgmc, aJgId, zje, PriLevel, PriLx1, PriStop, fklx, PriStopXx);
            }
            else
            {
                FrmYdWfkDsk1 f = new FrmYdWfkDsk1();
                f.ShowDialog();
                f.Prepare(ajgmc, aJgId, zje, PriDays, PriLx1, PriLx2, fklx, PriStopXx, PriStop);
            }
        }
    }
}