...
{
    public partial class FrmWFKDSKCX2 : UserControl
    {
        #region 成员变量
        private ClsG ObjG;
        ...
        private void BtnSearch_Click(object sender, EventArgs e)
        {
            ...
            PriLx2 = PriLx1;
            double cyje;
            if (!double.TryParse(this.TxtCyje.Text.Trim(), out cyje))
            {
                ClsMsgBox.Ts("差异金额输入格式错误");
                return;
            }
            //Dgv.Columns.Clear();
            //Dgv.DataSource = null;
            using (SqlConnection conn = new SqlConnection(ClsGlobals.CntStrTMS))
            {
                ...
                als.Add(new SqlParameter("@cyje", cyje));
                using (SqlCommand cmd = new SqlCommand())
                {
                    cmd.CommandText = PriLevel < 3 ? "P_dskwfkcx5" : "P_dskwfkcx7";
                    cmd.Connection = conn;
                    ...
                    if (dt.Rows.Count != 0)
                    {
                        ...
                        dr["yjx"] = dt.Compute("sum(yjx)", "").ToString();
                        dr["dbc"] = dt.Compute("sum(dbc)", "").ToString();
                        dr["level"] = 3;
                        dr["jgid"] = PriJgid;
                        Pridbc = Convert.ToDouble(dr["dbc"]);
                        dt.Rows.Add(dr);
                    }
                    else
        ...
        {
            string SQLstr = " SELECT dqmc, bh, sljgmc, slsj, qsd, mdd, qsjgmc, fhf, shlxr, ddsj,(case when (yqts+" + PriDays + ")<=1 then 0 else (yqts+" + PriDays + "-1) end) as yqts, lx, dsk,fkfs,sjzt,sjsj,mc,zhmc,zh  FROM dbo.Vfmswfkdskcx where yqts+ " + PriDays + " >=0 and jgid in (select id from jyjckj.dbo.tjigou WHERE parentLst LIKE '%," + PriJgid + ",%')";
        ...
        private void Dgv_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (PriLevel < 3)
            {
                FrmLSDWFKDSK2 f = new FrmLSDWFKDSK2();
                f.ShowDialog();
                f.Prepare(ajgmc, aJgId, zje, PriLevel, PriLx1, PriStop);
            }
            else
            {
                FrmYDWFKDSK2 f = new FrmYDWFKDSK2();
                f.ShowDialog();
                f.Prepare(ajgmc, aJgId, zje, PriDays, PriLx1, PriLx2, Pridbc);
            }
        }
    }
}