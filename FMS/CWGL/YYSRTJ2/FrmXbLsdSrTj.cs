...
{
    public partial class FrmXbLsdSrTj : Form
    {
        public FrmXbLsdSrTj()
        {
            InitializeComponent();
        ...
        public void Prepare(int aSljgid, DateTime aStartSj, DateTime aStopSj, int lx)
        {
            LblHj.Text = "合计：0元";
            ...
            if (lx == 1)
            {
                dt = ClsRWMSSQLDb.GetDataTable("  select mc,ydid,inssj,sljgid,ywqy,bh,fhxf,bf,jezj from Vfmsyysrcx3  " + aWhere, ClsGlobals.CntStrTMS);
            }
            else
            {
                dt = ClsRWMSSQLDb.GetDataTable("  select mc,ydid,inssj,sljgid,ywqy,bh,fhxf,bf,jezj from Vfmsyysrcx4  " + aWhere, ClsGlobals.CntStrTMS);
            }
